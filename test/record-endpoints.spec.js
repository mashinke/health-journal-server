...
describe('Record Endpoints', function () {
  ...
  describe('POST /api/record', () => {
    ...
    describe('Given a valid record', () => {
      ...
      it('stores the new record in db', () => supertest(app)
        .post('/api/record')
        .set(auth)
        .send(postAttemptBody)
        .then((res) => db
          ...
          .first()
          .then((row) => expect(row.values).to.eql(postAttemptBody.values))));
    });
  });
  ...
  describe('GET /api/record', () => {
    ...
    it('returns all user records', () => supertest(app)
      .get('/api/record')
      .set(auth)
      .expect(200)
      .expect((res) => {
        expect(res.body).to.be.an('array');
        res.body.forEach((record) => expect(record).to.include.all.keys(
          'id',
          'name',
          'values',
          'fields',
